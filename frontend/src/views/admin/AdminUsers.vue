    ...
    <template #header>
      <div class="admin-toolbar">
        <div class="admin-toolbar-left">
          <el-input v-model="query.keyword" placeholder="搜索用户/邮箱/手机号" clearable style="width: 240px" />
...
import { adminBatchUpdateUserStatus, adminListUsers, adminUpdateUserStatus } from '@/api/admin'
...
const toggleStatus = async (row) => {
  await adminUpdateUserStatus(row.id, row.status === 1 ? 0 : 1)
  ElMessage.success('状态更新成功')
  fetchList()
}

const batchUpdateStatus = async (status) => {
  await adminBatchUpdateUserStatus(selectedIds.value, status)
